...
class DengueNet_GRU(nn.Module):
    """DengueNet with BiGRU for temporal modeling"""

    def __init__(self, window_size, gru_hidden=256, gru_layers=3, dropout=0.3):
        super().__init__()
        ...
        # Attention mechanism
        self.attention = nn.MultiheadAttention(gru_hidden * 2, num_heads=8, batch_first=True)

        # Final prediction layers
        combined_size = gru_hidden * 2 * 3
        self.fc = nn.Sequential(
    ...
    def forward(self, rgb_images, radiomics, historical_cases):
        ...
        # GRU processing
        vit_out, _ = self.vit_gru(vit_features)
        radiomics_out, _ = self.radiomics_gru(radiomics_features)
        cases_out, _ = self.cases_gru(historical_cases.unsqueeze(-1))
...
class Visualizer:
    ...
    def plot_time_series(predictions, actuals, dates_info, save_path):
        ...
        # Create subplots: 2 cities per row for better visibility
        fig, axes = plt.subplots(int(np.ceil(n_cities/2)), 2, figsize=(24, 6*int(np.ceil(n_cities/2))))
        if n_cities == 1:
            axes = np.array([axes])
        else:
            axes = axes.flatten()
    ...
    def create_summary_report(train_results, val_results, test_results, history, save_path):
        ...
        # Title
        fig.suptitle('ViT-GRU Training Summary Report', fontsize=18, fontweight='bold', y=0.98)
...
def main():
    print("\n" + "="*80)
    print(" "*25 + "ViT-GRU MODEL TRAINING")
    print("="*80)
    ...
    with open(os.path.join(Config.OUTPUT_DIR, 'metrics_summary.json'), 'w') as f:
        summary = {
            'train': {key: (value.tolist() if hasattr(value, 'tolist') else value) for key, value in train_results['metrics'].items()},
            'validation': {key: (value.tolist() if hasattr(value, 'tolist') else value) for key, value in val_results['metrics'].items()},
            'test': {key: (value.tolist() if hasattr(value, 'tolist') else value) for key, value in test_results['metrics'].items()},
            'per_city': {city: {key: (value.tolist() if hasattr(value, 'tolist') else value) for key, value in metrics.items()} for city, metrics in test_results['city_metrics'].items()}
        }
